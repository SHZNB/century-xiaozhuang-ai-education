...
for (const [file] of requiredDocs) {
  if (!await exists(file)) missingDocs.push(file);
}

if (missingDocs.length) {
  console.error("Launch package check failed:");
  for (const file of missingDocs) console.error(`- 缺少 ${file}，请先运行 npm run acceptance、npm run check:env:report、npm run check:workflow-env:report 和 npm run check:deploy:report。`);
  process.exit(1);
}

const deployment = await readFile(path.join(root, "docs/DEPLOYMENT_CHECK.md"), "utf8");
const acceptance = await readFile(path.join(root, "docs/ACCEPTANCE.md"), "utf8");
const report = renderLaunchPackage({ deployment, acceptance });

if (shouldWrite) {
  await mkdir(path.join(root, "docs"), { recursive: true });
  await writeFile(path.join(root, "docs/LAUNCH_PACKAGE.md"), report, "utf8");
  console.log("Launch handoff package written to docs/LAUNCH_PACKAGE.md");
}

console.log(`Launch package check passed: ${buildAgentCatalog().length} agents, ${teachingWorkflows.length} workflows, ${requiredDocs.length} source reports`);
...
function renderLaunchPackage({ deployment, acceptance }) {
  ...
  const workflowRows = teachingWorkflows.map(workflow => {
    const route = routeById(workflow.routeHint);
    const env = providerEnvironment(workflow.routeHint);
    const envNames = env ? [env.key, env.url, env.model].filter(Boolean).join("、") : "未配置";
    return `| ${workflow.title} | ${route?.name || workflow.routeHint} | ${workflow.artifactType} | ${envNames} | ${workflow.inputFields?.length || 0}/${workflow.outputSections?.length || 0}/${workflow.qualityChecklist?.length || 0} |`;
  }).join("\n");
  const routeRows = requiredRoutes.map(([task, routeId]) => {
    const route = routeById(routeId);
    const env = providerEnvironment(routeId);
    return `| ${task} | ${route?.name || routeId} | ${env ? [env.key, env.url, env.model].filter(Boolean).join("、") : "按服务端配置"} |`;
  }).join("\n");
  const deploySummary = deployment.match(/- 通过：\d+\n- 提醒：\d+\n- 阻断：\d+/)?.[0] || "详见 docs/DEPLOYMENT_CHECK.md";
  const acceptanceSummary = acceptance.match(/## 汇总[\s\S]*?## 明细/)?.[0]?.replace("## 明细", "").trim() || "详见 docs/ACCEPTANCE.md";

  return `# 百年晓庄智慧教育平台上线交接包
...
- 平台名称：百年晓庄智慧教育平台
- 智能体目录：${agents.length} 个带 Logo 标识的教育智能体
- 教学工作流：${teachingWorkflows.length} 条，可在后台编辑、测试、发布、停用、导入和导出
...
| 报告 | 用途 |
| --- | --- |
${requiredDocs.map(([file, label]) => `| ${file} | ${label} |`).join("\n")}
